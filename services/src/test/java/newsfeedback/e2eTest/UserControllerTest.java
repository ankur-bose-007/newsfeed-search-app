...
import com.newsfeedback.core.Application;
...
public class UserControllerTest {
	...
	public void successfulLogin() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/user/login").content(mapper.writeValueAsString(user1))
				.contentType("application/json;charset=UTF-8")).andExpect(MockMvcResultMatchers.status().isOk());
	}
	...
	public void userBlocked() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/user/login").content(mapper.writeValueAsString(user1))
				.contentType("application/json;charset=UTF-8")).andExpect(MockMvcResultMatchers.status().isBadRequest())
	...
	public void successfulSignup() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/user/signup").content(mapper.writeValueAsString(user1))
				.contentType("application/json;charset=UTF-8")).andExpect(MockMvcResultMatchers.status().isCreated())
	...
	public void existingUserSignup() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/user/signup").content(mapper.writeValueAsString(user1))
				.contentType("application/json;charset=UTF-8")).andExpect(MockMvcResultMatchers.status().isConflict())
	...
	public void successfulGetUserDetails() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/user/getUserDetails/bose.ankur007@gmail.com").header("Authorization",
						"Bearer eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJqZWV0LmFua3VyMDA3QGdtYWlsLmNvbSIsImlhdCI6MTU1MDY0Njk2MCwiZXhwIjoxNTUxNTEwOTYwfQ.G2IBN8G1U8UoRHxVeHQOJu9JJEW07oNDWiLJxYP5gEvwGrxtMsapwU6GuU5yXnvq3SZqi8P9giAktxDkU5wvfg"))
	...
	public void nullEmailIdGetSearchHistory() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/user/searchHistory").content(mapper.writeValueAsString(user1))
				.contentType("application/json;charset=UTF-8")).andExpect(MockMvcResultMatchers.status().isNotFound());
	}
	...
	public void emptyEmailAddSearch() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.put("/user/addSearch/''").content(mapper.writeValueAsString(search))
				.contentType("application/json;charset=UTF-8"))
				.andExpect(MockMvcResultMatchers.status().isBadGateway());
	}
